...
function App() {
  ...
  // --- YOUR ORIGINAL GEO API LOGIC (UNTOUCHED) ---
  useEffect(() => {
    if (!query) {
      setSuggestions([]);
      return;
  ...
  let fetchWeatherData = async (latitude, longitude) => {
    // Added https to ensure it works on GitHub Pages
        ...
        <div>
          <div className='flex flex-col gap-4'>
            <div className='flex flex-col gap-4 min-[426px]:gap-10'>
              <div className='relative flex justify-center items-center'>
                {/* FIXED: Removed leading / from image paths */}
              ...
              <div className='grid grid-cols-[repeat(auto-fit,minmax(150px,1fr))] gap-4 min-[769px]:gap-1 justify-items-center items-center'>
                {data.hourly &&
                  <>
                    ...
                    <Item title={"Precipitation"} info={`${data.hourly.precipitation[new Date().getHours()]} mm`} />
                  </>
                }
              </div>
            </div>
          ...
          <div>
            <div className='text-left w-full p-2 box-border mt-4 mb-2'>
              <h1 className='text-3xl text-left '>Daily forecast</h1>
            </div>
            <div className='grid grid-cols-[repeat(auto-fit,minmax(105px,1fr))] gap-2 items-center justify-items-center'>
              {data.daily &&
                data.daily.weathercode.map((wc, index) => {
                  return <Card key={index} code={wc} day={weekDays[index].dayName} max={data.daily.temperature_2m_max[index]} min={data.daily.temperature_2m_min[index]} />;
                })
              }
            </div>
          </div>
